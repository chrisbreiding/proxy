const { appendBlockChildren, getBlockChildren, updateBlock } = require('./util')
...
const addFollowingWeekAndUpdateButton = async ({ query, params }) => {
  const { weekTemplatePageId, notionToken, startDate, addFollowingWeekButtonId } = query
  ...
  await appendBlockChildren({ notionToken, blocks, pageId: addFollowingWeekButtonId })
  await updateAddFollowingWeekButton({ query, params, dates })
}

const updateAddFollowingWeekButton = ({ query, params, dates }) => {
  const { notionToken, addFollowingWeekButtonId } = query
  ...
  return updateBlock({ notionToken, block, blockId: addFollowingWeekButtonId })
}
