...
const makeRequest = ({ notionToken, body, path, method = 'get' }) => {
  return request({
    method,
    body,
...
const textFilter = (block) => {
  return !!block[block.type].rich_text[0].plain_text.trim()
}
...
const getPlainText = (block, hasRichText = true) => {
  if (!block) return
  ...
  const textArray = hasRichText ? withText.rich_text : withText
