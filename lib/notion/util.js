...
const getBlockContent = async ({ notionToken, block, filter }) => {
  if (filter && !filter(block)) return

  const children = block.has_children
    ? (await getBlockChildrenDeep({ notionToken, pageId: block.id }))
    : undefined
...
const getBlockChildrenDeep = async ({ notionToken, pageId, filter }) => {
  const { results } = await getBlockChildren({ notionToken, pageId })

  const blocks = await Promise.all(results.map((block) => {
    return getBlockContent({ notionToken, block, filter })
  }))
...
const getPlainText = (block) => {
  ...
  if (!withText || (!withText.rich_text && !withText.text)) return

  return (withText.rich_text || withText.text)
