...
const persistentData = new PersistentData('notion-data')
...
const getCategories = async () => {
  // FIXME: these list ids are saved differently now
  ...
  try {
    ...
    debugVerbose('error getting stores: %s', error.stack)
...
const getStoreItems = async ({ storeId, parentId }) => {
  try {
    ...
    debugVerbose('error getting items: %s\n\n%o', error.stack, { storeId, parentId })
...
const broadcast = (message, data, options = {}) => {
  ...
  .forEach((id) => {
    debugVerbose('emit %s: %o', message, { socketId: id, items: data })
    sockets[id].emit(message, data)
  })
}

const onSocket = (socket) => {
  debugVerbose('socket connection: %o', { socketId: socket.id })
  ...
  socket.on('disconnect', () => {
    debugVerbose('socket disconnect: %o', { socketId: socket.id })

    delete sockets[socket.id]
  })

  socket.on('get:stores', async () => {
    debugVerbose('on get:stores: %o', { socketId: socket.id })
    ...
    debugVerbose('emit stores: %o', categories)

    socket.emit('stores', JSON.stringify(categories))
  })

  socket.on('sync:items', async ({ storeId, parentId }) => {
    debugVerbose('on sync:items: %o', { socketId: socket.id, storeId, parentId })
  ...
  socket.on('add:item', async ({ storeId }) => {
    debugVerbose('on add:item: %o', { socketId: socket.id, storeId })
  ...
  socket.on('update:item', async ({ storeId, item }) => {
    debugVerbose('on update:item: %o', { socketId: socket.id, storeId, item })

    const persistedData = await persistentData.get()
    // TODO: use util.areIdsEqual
  ...
  socket.on('update:items', async ({ storeId, items }) => {
    debugVerbose('on update:items: %o', { socketId: socket.id, storeId, items: items.map((item) => item.id) })
  ...
  socket.on('delete:item', async ({ storeId, itemId }) => {
    debugVerbose('on delete:item: %o', { socketId: socket.id, storeId, itemId })
