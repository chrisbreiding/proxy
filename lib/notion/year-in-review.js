...
const getPageIds = async ({ donePageId, notionToken, year }) => {
...
const getMonthData = async ({ month, notionToken, data }) => {
...
const getData = async ({ months, notionToken }) => {
  let data = []

  for (let month of months) {
    data = await getMonthData({ month, notionToken, data })
  }
...
const yearInReview = async ({ donePageId, notionToken, year }) => {
  ...
  await makeAppendRequest({ notionToken, blocks, pageId: yearId })
}
